...
func newHTTPEncoder(payload *network.Connections) *httpEncoder {
	...
	dataCardinality := min(len(payload.Conns), len(payload.HTTP))
	encoder := &httpEncoder{
		aggregations: make(map[http.Key]*model.HTTPAggregations, len(payload.Conns)),

		// pre-allocate all data objects at once
		dataPool: make([]model.HTTPStats_Data, dataCardinality*http.NumStatusClasses),
		ptrPool:  make([]*model.HTTPStats_Data, dataCardinality*http.NumStatusClasses),
		poolIdx:  0,
	}
